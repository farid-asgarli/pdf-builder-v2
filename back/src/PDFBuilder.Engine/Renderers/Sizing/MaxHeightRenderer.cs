...
/// <summary>
/// Renders a MaxHeight wrapper component that applies a maximum height constraint to its child content.
/// </summary>
/// <remarks>
/// QuestPDF MaxHeight API:
/// - MaxHeight(value) - Sets the maximum height of its content.
///
/// Properties:
/// - value (float): The maximum height value in points. Required.
...
/// </remarks>
/// <remarks>
/// Initializes a new instance of the <see cref="MaxHeightRenderer"/> class.
/// </remarks>
...
public sealed class MaxHeightRenderer(
    IExpressionEvaluator expressionEvaluator,
    StyleResolver styleResolver,
    ILogger<MaxHeightRenderer> logger
) : BaseRenderer(expressionEvaluator, styleResolver, logger)
{
    ...
    /// <inheritdoc />
    public override ComponentType ComponentType => ComponentType.MaxHeight;
    ...
    )
    {
        ...
        {
            Logger.LogWarning(
                "MaxHeight value is null or non-positive for node {NodeId}, skipping height constraint",
                node.Id ?? "unnamed"
            );
        ...
        Logger.LogTrace(
            "Rendering MaxHeight with value={Value}pt (original: {OriginalValue}{Unit})",
            valueInPoints,
            value,
        ...
        IContainer constrainedContainer = container.MaxHeight(valueInPoints);
        ...
        if (value is not null && value <= 0)
        {
            errors.Add(
                new ComponentValidationError
                {
                    ...
                    Message = $"MaxHeight value must be positive, got: {value}",
