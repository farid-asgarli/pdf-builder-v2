...
/// <summary>
/// Renders a MaxWidth wrapper component that applies a maximum width constraint to its child content.
/// </summary>
/// <remarks>
/// QuestPDF MaxWidth API:
/// - MaxWidth(value) - Sets the maximum width of its content.
///
/// Properties:
/// - value (float): The maximum width value in points. Required.
...
/// </remarks>
/// <remarks>
/// Initializes a new instance of the <see cref="MaxWidthRenderer"/> class.
/// </remarks>
...
public sealed class MaxWidthRenderer(
    IExpressionEvaluator expressionEvaluator,
    StyleResolver styleResolver,
    ILogger<MaxWidthRenderer> logger
) : BaseRenderer(expressionEvaluator, styleResolver, logger)
{
    ...
    /// <inheritdoc />
    public override ComponentType ComponentType => ComponentType.MaxWidth;
    ...
    )
    {
        ...
        {
            Logger.LogWarning(
                "MaxWidth value is null or non-positive for node {NodeId}, skipping width constraint",
                node.Id ?? "unnamed"
            );
        ...
        Logger.LogTrace(
            "Rendering MaxWidth with value={Value}pt (original: {OriginalValue}{Unit})",
            valueInPoints,
            value,
        ...
        IContainer constrainedContainer = container.MaxWidth(valueInPoints);
        ...
        if (value is not null && value <= 0)
        {
            errors.Add(
                new ComponentValidationError
                {
                    ...
                    Message = $"MaxWidth value must be positive, got: {value}",
