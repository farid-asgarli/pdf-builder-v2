...
/// <summary>
/// Renders a MinHeight wrapper component that applies a minimum height constraint to its child content.
/// </summary>
/// <remarks>
/// QuestPDF MinHeight API:
/// - MinHeight(value) - Sets the minimum height of its content.
///
/// Properties:
/// - value (float): The minimum height value in points. Required.
...
/// </remarks>
/// <remarks>
/// Initializes a new instance of the <see cref="MinHeightRenderer"/> class.
/// </remarks>
...
public sealed class MinHeightRenderer(
    IExpressionEvaluator expressionEvaluator,
    StyleResolver styleResolver,
    ILogger<MinHeightRenderer> logger
) : BaseRenderer(expressionEvaluator, styleResolver, logger)
{
    ...
    /// <inheritdoc />
    public override ComponentType ComponentType => ComponentType.MinHeight;
    ...
    )
    {
        ...
        {
            Logger.LogWarning(
                "MinHeight value is null or non-positive for node {NodeId}, skipping height constraint",
                node.Id ?? "unnamed"
            );
        ...
        Logger.LogTrace(
            "Rendering MinHeight with value={Value}pt (original: {OriginalValue}{Unit})",
            valueInPoints,
            value,
        ...
        IContainer constrainedContainer = container.MinHeight(valueInPoints);
        ...
        if (value is not null && value <= 0)
        {
            errors.Add(
                new ComponentValidationError
                {
                    ...
                    Message = $"MinHeight value must be positive, got: {value}",
