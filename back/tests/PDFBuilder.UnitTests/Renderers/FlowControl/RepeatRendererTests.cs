...
/// <summary>
/// Unit tests for the RepeatRenderer class.
/// </summary>
public sealed class RepeatRendererTests : IDisposable
{
    private readonly Mock<ILogger<RepeatRenderer>> _loggerMock;
    ...
    private readonly RepeatRenderer _renderer;

    public RepeatRendererTests()
    {
        _loggerMock = new Mock<ILogger<RepeatRenderer>>();
        ...
        _renderer = new RepeatRenderer(_expressionEvaluator, _styleResolver, _loggerMock.Object);
    }
    ...
    [Fact]
    public void ComponentType_ShouldBeRepeat()
    {
        // Assert
        _renderer.ComponentType.Should().Be(ComponentType.Repeat);
    }

    [Fact]
    public void RendererName_ShouldBeRepeatRenderer()
    {
        // Assert
        _renderer.RendererName.Should().Be("RepeatRenderer");
    }
    ...
    {
        // Arrange
        var node = new LayoutNode { Type = ComponentType.Repeat, Id = "test-repeat" };
    ...
    public void Render_WithChild_ShouldRenderChildViaLayoutEngine()
    {
        ...
        // Assert - Repeat should call layout engine to render its child
